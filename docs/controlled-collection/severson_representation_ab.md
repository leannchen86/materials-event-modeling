...
> **Current requalification (2026-07-12).** Right-censored cells are `unknown` plus
> `cell.record_truncated=true`, not negative or ambiguous outcomes; the corrected cumulative
> conformance grade is L1, while the non-cumulative L3 structure check still passes incidentally.
> The A/B root was an adapted per-cycle record that omitted within-cycle arrays. Within-corpus
> ranking signal survived, but the ranking rule failed held-batch transfer. Historical
> preregistration and result text below is retained for auditability.
...
grammar-adapted Severson batch 1 (46 cells; historical grade later corrected in the
...
should be set by adapters using simple physical-bounds checks, so complete retained records
...
- Batches 2–3 downloaded (checksums in `data/manifests/severson_download.json`); adapter
  ...
  (128 EOL, 7 censored), 72 policies (31 replicated), cumulative grade L1. The non-cumulative L3
  structure check passes because of incidental channel variation. The literature's
...
paper shape is chance-bound; (3) a regression-level held-batch result for the linear model, not
ranking transfer. The ranking rule later collapsed to chance on the adequately powered held batch.
The batch-fingerprint finding (94.5%) connects rung 3 to the provenance-critique branch: adapted
representations are not automatically provenance-free.

Follow-ons, in priority order: (a) **DONE** — the provenance-leakage audit now runs
directly on the A-representation matrices
([second-dataset replication](../provenance-critique/second_dataset_replication.md), severson_ab
