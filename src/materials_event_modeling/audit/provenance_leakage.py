"""Provenance-leakage audit (a.k.a. collection-artifact / contamination detection).
...
In a pretraining corpus that "incidental" label is the source/snapshot/benchmark a
document came from; if a feature representation lets you recover it well above chance,
the representation has encoded provenance rather than signal, and a model trained on it
can shortcut — the same failure mode that benchmark *decontamination* exists to prevent.
In this repo the records are XRD/Raman spectra and the label is the originating
lab/instrument ("models learn the lab"): the diagnostic is mechanically identical.

The core is modality-agnostic. Callers pass:

* ``feature_sets`` — a mapping ``{name: matrix}`` of (n_items, n_features) arrays. A
  matrix can be PCA-reduced spectra, text embeddings, hashed n-grams — anything.
* ``labels`` — the per-item provenance/source id to try to recover.

and get back, per feature set, a recoverability measurement plus a normalized
``leakage_score`` in ``[0, 1]`` and a ``severity`` verdict — the go/no-go trust
artifact a data buyer asks for.
...
def audit_feature_sets(
    ...
    *,
    ...
    """Audit every feature set for provenance leakage and rank by severity.
    ...
    recommendation = {
        ...
        "clean": (
            ...
            "audited feature sets."
        ),
    }[worst_severity]

    return {
        "task": "provenance_leakage_audit",
        ...
        "caveats": [
            ...
            "for that).",
            "High recoverability means features can encode lab/instrument/snapshot "
            "identity; it does not by itself prove the downstream task is contaminated.",
        ],
    }
...
def control_efficacy(
    ...
    """How much does a control feature set reduce leakage vs a baseline feature set?

    e.g. baseline=``full_xrd_pca`` vs control=``crop_xrd_derivative_pca`` answers
    "did the preprocessing control actually neutralize the source confound?" — the
    decontamination-remediation check.
    """
